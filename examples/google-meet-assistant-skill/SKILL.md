...
- **By default `meet_create` also auto-joins and starts recording** the new room (the bot owns it on its org account, so it enters with no lobby). You do NOT need a separate `meet_join` call for the create-and-record flow. Its result already carries `joined: true` — **read that and move on. Do NOT follow up with `meet_status`**: it reports nothing `meet_create` did not already tell you, and every extra tool call is a full LLM round-trip on the user's clock. Call `meet_status` here only when `joined` is false or missing.
...
- Prefer `meet_create` for room creation. It selects its own backend (`HERMES_MEET_CREATE_MODE`, default `auto`): the **Meet REST API** when an OAuth credential carrying the `meetings.space.created` scope is present (~2s), otherwise the **Meet/Chrome signed-in session** (~9s). Either way this needs nothing from you, and a missing OAuth credential does not block the path — it simply falls back to the browser.
...
- **That session expires, and its death looks like a lobby timeout.** A Google Workspace *web session duration* policy force-expires it on a fixed cycle (14 days by default), and Google can revoke it earlier. A signed-out bot silently degrades to a guest and sits in the lobby — so `leaveReason: "lobby_timeout"` is often really "the session died", not "the host ignored us". **Do not judge session health by cookie expiry dates**: the cookies on disk stay valid for months after Google has revoked them server-side. Re-auth with `hermes meet auth`, or — on a headless box, where that command has no display to open — with the plugin's `tools/meet-auth-vnc`.
...
- **Hide the plumbing — just omit the developer-facing details.** Report naturally; the only rule is to leave out the technical internals, not to use any fixed phrasing. Don't surface raw tool output/JSON, internal status field names (`inCall`, `transcriptPath`, `leaveReason`, `captionLanguage`), or absolute file paths like `/home/.../meetings/<code>/transcript.txt`. Those are for *your* own verification; phrase the result for the user however fits the moment.
