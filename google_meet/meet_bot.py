...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    try:
        with sync_playwright() as pw:
            ...
            # Dead session: the goto never reached Meet at all — Google
            # redirected to the sign-in flow. Bail out immediately with the
            # actionable error; none of the pre-join / lobby detectors below
            # can see a page that isn't Meet, so without this the run stalls
            # until the lobby timeout and blames the host.
            if _detect_signed_out_redirect(page):
                state.set(
                    error=_NOT_AUTHED_ERROR,
                ...
                try:
                    ...
                    pass
                # Realtime mode sets the audio bridge up *before* the browser
                # launch, so an early exit here has to tear it down or the
                # virtual sink leaks.
                if rt.get("bridge"):
                    try:
                        rt["bridge"].teardown()
                ...
                if not state.in_call and (now - last_admission_check) > 3.0:
                    ...
                    if admitted:
                        ...
                        try:
                            ...
                            pass
                    elif _detect_signed_out_redirect(page):
                        # Session died after the initial load (or the redirect
                        # landed late). Same fail-fast as the pre-join check —
                        # no point waiting out the lobby on a sign-in page.
                        state.set(
                            error=_NOT_AUTHED_ERROR,
                            leave_reason="not_authenticated",
                        )
...
_NOT_AUTHED_ERROR = (
    "not signed in — the saved Google session is invalid or expired. "
    "Re-authenticate with: hermes meet auth "
    "(headless box: google_meet/tools/meet-auth-vnc)"
)


def _detect_signed_out_redirect(page) -> bool:
    """True when Meet bounced us off to the Google sign-in flow.

    A fully revoked session doesn't render *any* Meet screen: the navigation
    is redirected to ``accounts.google.com/…/signin|confirmidentifier``, so
    the DOM probes below (which all read Meet's own UI) see nothing they
    recognise and the bot would otherwise sit out the whole lobby timeout and
    report the misleading ``lobby_timeout``. Distinct from
    :func:`_detect_guest_screen`, which is the *partially* signed-out case
    where Meet still loads and offers the anonymous 'Ask to join' path.

    Reads ``location.href`` via ``evaluate`` rather than ``page.url``: the
    latter is a client-side cached value and goes stale across a redirect.
    Conservative: False on error.
    """
    try:
        href = page.evaluate("() => location.href") or ""
    except Exception:
        return False
    return "accounts.google.com" in href.split("?", 1)[0]
