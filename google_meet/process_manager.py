...
def _prune_old_meetings(keep_days: float, current: Path) -> None:
    """Delete meeting folders whose last activity is older than *keep_days*.

    Transcripts can hold sensitive content and otherwise pile up forever, so
    we garbage-collect on each new join. Only ``<meeting-id>/`` directories are
    touched — never files like ``auth.json`` or ``.active.json`` — and never
    the meeting being recorded right now. ``keep_days <= 0`` disables pruning.
    """
    if keep_days <= 0:
        return
    root = _root()
    ...
    for child in root.iterdir():
        try:
            if not child.is_dir():
                continue  # auth.json / .active.json are files — leave them
            if child.resolve() == current_resolved:
                continue  # never delete the active recording
...
def start(
    ...
    *,
    ...
    # Retention: prune old meeting folders so transcripts (which can hold
    # sensitive content) don't accumulate forever. Default 30 days; set
    # HERMES_MEET_RETENTION_DAYS=0 to keep everything.
    _prune_old_meetings(
        float(os.environ.get("HERMES_MEET_RETENTION_DAYS", "30")), out
    )

    # Always continue the same transcript: a leave+rejoin (or a later meeting on
    # the same Meet code) APPENDS to transcript.txt / raw_captions.jsonl rather
    # than wiping them — we never delete prior content. Only status.json (live
    # state) is cleared so polling never sees a previous run's state.
    sf = out / "status.json"
    if sf.exists():
        try:
            sf.unlink()
    ...
    env = os.environ.copy()
    # This is a user-override plugin living at $HERMES_HOME/plugins/google_meet,
    # so the bot subprocess (`-m google_meet.meet_bot`) needs that dir on the
    # import path. Prepend it so the override's modules resolve (not the bundled).
    _plugins_dir = str(Path(__file__).resolve().parent.parent)
    env["PYTHONPATH"] = _plugins_dir + (
    ...
    # Authenticate by default: the agent/Discord tool path doesn't pass an
    # auth_state, but org meetings block anonymous joins. Fall back to the
    # standard signed-in session so tool-triggered joins authenticate too.
    if not auth_state:
        _cand = _root() / "auth.json"
        if _cand.is_file():
            auth_state = str(_cand)
    if auth_state:
    ...
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "google_meet.meet_bot"],
