...
MEET_JOIN_SCHEMA: Dict[str, Any] = {
    "name": "meet_join",
    "description": (
        "Join/record a Google Meet call (use for '회의 기록해줘', '이 회의 "
        "들어와줘', 'transcribe this meet'). Starts scraping live captions into "
        "a transcript file. Only meet.google.com URLs are accepted; no calendar "
...
MEET_CREATE_SCHEMA: Dict[str, Any] = {
    "name": "meet_create",
    "description": (
        "Create a brand-new Google Meet link (use for '회의 만들어줘', '미트 "
        "열어줘', 'make a meet link') using the bot's signed-in account. Returns "
        "the shareable meeting URL to post (e.g. in a Discord channel). "
        "The bot ALWAYS auto-joins the new room and starts recording by "
        "default (it created the room on its own org account, so it enters "
        "with no lobby) — this is the expected behavior on '회의 만들어줘' / "
        "'make a meet'. Just call it with no extra params and the bot enters "
        "immediately. ONLY set link_only=true when the user EXPLICITLY wants a "
        "shareable link with NO bot in the room (e.g. '회의 링크만 만들어줘', "
        "'just give me a link, don't join')."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "link_only": {
                "type": "boolean",
                "description": (
                    "Default false. Leave UNSET for normal 'make a meeting' "
                    "requests — the bot auto-joins and records immediately. "
                    "Set true ONLY when the user explicitly wants just a "
                    "shareable link with NO bot in the room (e.g. '회의 링크만 "
                    "만들어줘', 'just give me a link, don't join')."
                ),
            },
            "mode": {
                "type": "string",
                "enum": ["transcribe", "realtime"],
                "description": (
                    "Mode when the bot joins. transcribe (default) = "
                    "listen-only captions; realtime = also speak via meet_say."
                ),
            },
            "duration": {
                "type": "string",
                "description": (
                    "Optional max duration before auto-leave (e.g. '30m', "
                    "'2h'). Omit to stay until meet_leave."
                ),
            },
        },
        "additionalProperties": False,
...
def handle_meet_create(args: Dict[str, Any], **_kw) -> str:
    """Create a new Meet link via the bot's signed-in browser session.

    Runs the short ``meet_create`` Playwright subprocess (blocking, ~15-30s)
    and returns the generated URL. No GCP/OAuth — reuses the join auth session.
    """
    import os
    import subprocess
    import sys
    ...
    env = os.environ.copy()
    env["PYTHONPATH"] = plugins_dir + (
        os.pathsep + env["PYTHONPATH"] if env.get("PYTHONPATH") else ""
    )
    try:
        proc = subprocess.run(
            [sys.executable, "-m", "google_meet.meet_create"],
            env=env, capture_output=True, text=True, timeout=120,
        )
    except subprocess.TimeoutExpired:
        return _err("meet_create timed out — the browser took too long")

    url = None
    for line in (proc.stdout or "").splitlines():
        if line.startswith("MEET_URL="):
            url = line[len("MEET_URL="):].strip()
            break
    if not url:
        detail = (proc.stderr or proc.stdout or "no output").strip()[-300:]
        return _err(f"meet_create failed: {detail}")

    out: Dict[str, Any] = {"success": True, "url": url}

    # Auto-join + record the freshly created room. The bot owns the room
    # (created on its signed-in org account), so it enters with no lobby.
    #
    # We deliberately do NOT gate this on a `join` flag. Models routinely
    # mis-send join=false on a plain "회의 만들어줘", which left the bot out of
    # its OWN room — the exact bug this fixes. The only way to get a link
    # without the bot is an explicit link_only=true (the rare "링크만" case);
    # a stray join=false is ignored so create-AND-join is the reliable default.
    link_only = bool(args.get("link_only", False))
    if not link_only:
        join_args: Dict[str, Any] = {"url": url}
        for k in ("mode", "duration", "node"):
            if args.get(k) is not None:
                join_args[k] = args[k]
        try:
            jr = json.loads(handle_meet_join(join_args))
        except Exception as e:  # pragma: no cover — defensive
            jr = {"success": False, "error": f"join result unparseable: {e}"}
        out["joined"] = bool(jr.get("success"))
        out["join"] = jr
    else:
        out["joined"] = False
        out["link_only"] = True
