...
| Tool | Parameters | Use |
|---|---|---|
| `meet_create` | `link_only?`, `mode?`, `duration?` | Create a new Meet link. **Always auto-joins + records by default**; set `link_only=true` for a link with no bot in the room. |
...
| `leaveReason` | `duration_expired`, `lobby_timeout`, `not_authenticated`, `denied`, `page_closed`, or null. `not_authenticated` = session invalid, re-run `hermes meet auth` (do not re-join blindly). |
